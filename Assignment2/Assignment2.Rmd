...
rf_mtry <- train(diabetes ~ pregnant + glucose + pressure + triceps +insulin + mass
    + pedigree + age,
    data = data_train,
    method = "rf",
    metric = "Accuracy",
...
#try different maxnode
store_maxnode <- list()
tuneGrid <- expand.grid(.mtry = best_mtry)
for (maxnodes in c(5: 15)) {
    rf_maxnode <- train(diabetes ~ pregnant + glucose + pressure + triceps +insulin +       mass + pedigree + age,
    data = data_train,
        method = "rf",
        metric = "Accuracy",
        tuneGrid = tuneGrid,
...
for (ntree in c(250, 300, 350, 400, 450, 500, 550, 600, 800, 1000, 2000)) {
      rf_maxtrees <- train(diabetes ~ pregnant + glucose + pressure + triceps +insulin +       mass + pedigree + age,
      data = data_train,
      method = "rf",
      metric = "Accuracy",
      tuneGrid = tuneGrid,
...
store_nodesize <- list()
tuneGrid <- expand.grid(.mtry = best_mtry)
for (nodesize in c(10: 30)) {
      rf_nodesize <- train(diabetes ~ pregnant + glucose + pressure + triceps +insulin +       mass + pedigree + age,
      data = data_train,
      method = "rf",
      metric = "Accuracy",
      tuneGrid = tuneGrid,
...
#Train the forest
fit_rf <- train(diabetes ~ pregnant + glucose + pressure + triceps +insulin +       mass     + pedigree + age,
    data = data_train,
    method = "rf",
    metric = "Accuracy",
    tuneGrid = tuneGrid,
...
#Evaluate
prediction_rf <- predict(fit_rf, newdata= data_test)
rf_matrix <- confusionMatrix(prediction_rf, data_test$diabetes)
rf_matrix
```
### Performance results
To be able to compare the performance of LR with other models, we construct the confusion matrix of predictions on the test data. From this confusion matrix, we compute the metrics that we think are most important.
