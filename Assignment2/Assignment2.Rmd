...
### Summary statistics
In order to get a grip of our data and to also determine whether the imputation has altered the properties of our dataset we will extract some summary statistics, and create some exploratory plots concerning the distribution of each value in healthy and diseased individuals. 
```{r, echo = FALSE}
...
```
These give us an insight in our dataset. We see for example that the glucose ranges between 44 and 199, with an average of 121.6, that we have individuals between the age of 21 and 81, that we have a person that has been pregnant 17 times! in the past and more. We also see that our dataset is imbalanced,with 500 negative and 268 positive individuals. 
...
### Models and Metrics
In our analysis we consider three models. We would classify two models as being simple: the logistic regression and the classification tree. As a more complex model, we consider the random forest. 

To be able to compare the performance of LR with other models, we construct the confusion matrix of predictions on the test data. Here, we will list the metrics that we will use, alongside the abbreviations we used in the rest of the document.
...
### McNemar's test
We have performed McNemar's test thrice: one time for each pair of models. As we were not sure which R function we could use, we have coded the test ourselves. Please view the .Rmd for this implementation.
...
The results of the three-fold McNemar test tell us that the difference in performance of the models is not significant. Of course we have to note that the difference is computed on just 154 cases: maybe the difference become significant when a larger (test) dataset is used. 