...
for (i in 1:length(numeric_features_indices))
    for (j in 1:length(categorical_features_indices))
    {
      plot <- data %>% 
                ggplot(aes_string(x=colnames(data[numeric_features_indices[i]]), fill=colnames(data[categorical_features_indices[j]]))) + 
                geom_density(alpha = .5, colour = NA) + theme_minimal()
      print(plot)
    }
