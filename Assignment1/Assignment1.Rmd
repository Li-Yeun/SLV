...
For the categorical variables, we tested too see if the key of the song have an affect on the peak_rank or the weeks_on_chart. From our common knowledge we know that most pop-songs are written in the C-key. Therefore, we assume that songs written in other keys will perform less on the charts. To confirm our suspicion we plotted the following box-plots.
```{r, echo=FALSE}
...
```{r, echo = FALSE }
#time signature
data%>%ggplot(aes(x=time_signature,y=peak_rank))+geom_boxplot()+theme_minimal()
```
...
When considering the first two variables of the dataset, peak rank and weeks on chart, one might expect that songs with a high peak rank will last more weeks on chart. For a song at rank 1, it may take more weeks to drop out the top 200 due to the high popularity of the song. To explore this thought, we made the plot investigating weeks on chart vs peak rank.  From the plot we see our thought confirmed, or at least up to some peak rank. For the really popular songs, songs with a rank between 1 and 50, it is clear that they stay on the chart for weeks than less popular songs do. We test this by performing a Pearsons correlation test. The test returned a correlation estimate of -0.3525126. This demonstrates a negative correlation since a higher peak rank is represented by a low numerical estimate. Furthermore, a “For loop” was written to test every numerical feature against another. This returned energy/loudness correlation of 0.69 p<2.2e-16 and an Energy/acousticness correlation of -0.59 p<2.2e-16.
