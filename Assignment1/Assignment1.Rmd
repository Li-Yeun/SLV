...
data%>%
  ...
  labs(x="Weeks on Chart", title  = "                                                               Figure 5") + 
  theme_minimal()
```
```{r, echo=FALSE}
ggplot(data,aes(y=energy,x=weeks_on_chart)) +geom_point() + stat_cor(label.x = 0.4, label.y = 0.1) +facet_wrap(~key) +theme_minimal()
```
